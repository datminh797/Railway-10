...
/* Câu hỏi 1: Đếm số lượng nhân viên của các phòng ban*/
select count(departmentname)  from department;
select DepartmentID from Account group by DepartmentID order by DepartmentID;
select departmentID, count(fullname) as So_Luong_NV from Account group by DepartmentID order by DepartmentID ;
/*select departmentID, count(fullname) as So_Luong_NV from Account group by DepartmentID having So_Luong_NV > 10 order by DepartmentID ;*/
/* select fullname, count(fullname) from Account where DepartmentID = 4;*/

/* Câu hỏi 2: Đếm số lượng nhân viên phòng ban Nhân sự*/
select * from department order by departmentid ;
/* select departmentid from department where departmentname = 'Nhân sự'; */
/* select departmentid, count(fullname) from account where departmentid = 4; */
select departmentid, count(fullname) from account group by departmentid having departmentid =
 (select departmentid from department where departmentname = 'Nhân Sự');
SELECT DepartmentID
FROM Department
WHERE DepartmentName = 'Nhân sự';


/* Câu hỏi 2: Đếm số lượng nhân viên phòng ban Nhân sự*/
SELECT DepartmentID, COUNT(*) AS 'Số lượng nhân viên'
FROM `Account`
GROUP BY DepartmentID
HAVING DepartmentID= (SELECT DepartmentID FROM Department WHERE DepartmentName = 'Nhân sự');

/*SELECT 	t1.column,  t2.column
FROM 	table1 t1
JOIN 		table2 t2	ON 	t1.column = t2.column*/
SELECT d.DepartmentID, d.DepartmentName, COUNT(username) AS 'Số nhân viên'
FROM department d JOIN `account` a ON d.DepartmentID = a.DepartmentID WHERE DepartmentName = 'Nhân sự';

/* Câu hỏi 3: Đếm số lượng nhân viên phòng Marketing và phòng Sale */
select departmentid, count(fullname) from account where departmentid = 1;
select departmentid, count(fullname) as So_NV from account having departmentid = 1 ; -- false 
select departmentid, count(fullname) as So_NV from account where departmentid = 1 ; -- true


select departmentid, count(*) as 'Số nhân viên' from account group by departmentid HAVING departmentid =
 (select departmentid from department where departmentname = 'Marketing') ;
/* Câu hỏi 4: Đếm xem mỗi phòng có bao nhiêu nhân viên Test*/
-- Làm bài ở đây
select departmentid, positionid, count(fullname) as 'Số nhân viên' from account
 group by departmentid, positionid having positionid =
(select positionid from position where positionname = 'Test' )
order by departmentid;


/* Câu hỏi 5: Đếm xem phòng Kỹ thuật có bao nhiêu nhân viên Test*/
-- Làm bài ở đây
select departmentid, positionid, count(*) as 'Số nhân viên'
from account group by departmentid, positionid having 
departmentid = (select departmentid from department where departmentname = 'Kỹ thuật')
